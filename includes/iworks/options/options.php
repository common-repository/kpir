...
class iworks_options {
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			...
			esc_html_e( 'An error occurred while getting the configuration.', 'kpir' );
			echo '</strong></p></div>';
			return;
		}
		/**
...
<?php } ?>
	...
	private function location( $name, $value = '', $args = array() ) {
		...
		$i18n     = array(
			'country' => esc_html__( 'Country', 'kpir' ),
			'city'    => esc_html__( 'City', 'kpir' ),
			'street'  => esc_html__( 'Street', 'kpir' ),
			'zip'     => esc_html__( 'ZIP code', 'kpir' ),
		);
		...
		$f = array(
			...
			array(
				...
				'wp_localize_script' => array(
					'buttons' => array(
						'select_media' => __( 'Select Image', 'kpir' ),
					),
				),
			),
			/**
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => esc_html__( 'OFF', 'kpir' ),
				'on_label'  => esc_html__( 'ON', 'kpir' ),
			),
		);
		return $data;
	}
	...
	private function get_nonce_value() {
		...
		return new WP_Error( 'security', esc_html__( 'Failed Security Check', 'kpir' ) );
	}
